...
import { BaseExamples } from '@vaif/api/resources/docs/examples';
...
const runTests = (client: PartialVaif<{ docs: { examples: BaseExamples } }>) => {
  test('retrieve', async () => {
    const responsePromise = client.docs.examples.retrieve('slug');
  ...
  test('list', async () => {
    const responsePromise = client.docs.examples.list();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
