...
import { BaseAuthorize } from '@vaif/api/resources/oauth/org/provider/authorize';
import { Provider } from '@vaif/api/resources/oauth/org/provider/provider';
...
const runTests = (client: PartialVaif<{ oauth: { org: { provider: { authorize: BaseAuthorize } } } }>) => {
  test('getAuthorize: only required params', async () => {
    const responsePromise = client.oauth.org.provider.authorize.getAuthorize('x', { orgId: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' });
  ...
  test('getAuthorize: required and optional params', async () => {
    const response = await client.oauth.org.provider.authorize.getAuthorize('x', { orgId: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' });
  });
};
