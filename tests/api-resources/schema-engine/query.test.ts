...
import { BaseQuery } from '@vaif/api/resources/schema-engine/query';
...
const runTests = (client: PartialVaif<{ schemaEngine: { query: BaseQuery } }>) => {
  test('create: only required params', async () => {
    const responsePromise = client.schemaEngine.query.create('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { sql: 'x' });
  ...
  test('create: required and optional params', async () => {
    const response = await client.schemaEngine.query.create('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { sql: 'x', params: [{}] });
  });
};
