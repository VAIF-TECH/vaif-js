...
import { AlertRules } from '@vaif/api/resources/alert-rules/alert-rules';
import { BaseProject } from '@vaif/api/resources/alert-rules/project';
...
const runTests = (client: PartialVaif<{ alertRules: { project: BaseProject } }>) => {
  ...
  test('retrieve', async () => {
    const responsePromise = client.alertRules.project.retrieve('projectId');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
