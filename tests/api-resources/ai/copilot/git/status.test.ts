...
import { BaseStatus } from '@vaif/api/resources/ai/copilot/git/status';
...
const runTests = (client: PartialVaif<{ ai: { copilot: { git: { status: BaseStatus } } } }>) => {
  test('retrieve', async () => {
    const responsePromise = client.ai.copilot.git.status.retrieve('sessionId');
