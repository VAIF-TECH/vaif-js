...
describe('resource verifyEmail', () => {
  ...
  test.skip('confirm: only required params', async () => {
    const responsePromise = client.auth.verifyEmail.confirm({ token: 'x' });
  ...
  test.skip('confirm: required and optional params', async () => {
    const response = await client.auth.verifyEmail.confirm({ token: 'x' });
  });

  // Mock server tests are disabled
  test.skip('send', async () => {
    const responsePromise = client.auth.verifyEmail.send();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
