...
import { Policies } from '@vaif/api/resources/storage/buckets/policies/policies';
import { BaseToggle } from '@vaif/api/resources/storage/buckets/policies/toggle';
...
const runTests = (client: PartialVaif<{ storage: { buckets: { policies: { toggle: BaseToggle } } } }>) => {
  test('toggle: only required params', async () => {
    const responsePromise = client.storage.buckets.policies.toggle.toggle('policyId', {
      bucketId: 'bucketId',
    });
  ...
  test('toggle: required and optional params', async () => {
    const response = await client.storage.buckets.policies.toggle.toggle('policyId', {
      bucketId: 'bucketId',
    });
  });
};
