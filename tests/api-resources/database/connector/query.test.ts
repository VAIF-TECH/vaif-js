...
import { Connector } from '@vaif/api/resources/database/connector/connector';
import { BaseQuery } from '@vaif/api/resources/database/connector/query';
...
const runTests = (client: PartialVaif<{ database: { connector: { query: BaseQuery } } }>) => {
  test('query: only required params', async () => {
    const responsePromise = client.database.connector.query.query('projectId', {
      table: 'x',
      wrapperId: 'x',
    });
