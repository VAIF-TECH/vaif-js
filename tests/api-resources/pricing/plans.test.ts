...
import { BasePlans } from '@vaif/api/resources/pricing/plans';
...
const runTests = (client: PartialVaif<{ pricing: { plans: BasePlans } }>) => {
  test('retrieve', async () => {
    const responsePromise = client.pricing.plans.retrieve('plan');
  ...
  test('list', async () => {
    const responsePromise = client.pricing.plans.list();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
