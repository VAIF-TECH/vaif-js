...
import { BaseChangePlan } from '@vaif/api/resources/billing/org/change-plan';
...
const runTests = (client: PartialVaif<{ billing: { org: { changePlan: BaseChangePlan } } }>) => {
  ...
  test('changePlan: required and optional params', async () => {
    const response = await client.billing.org.changePlan.changePlan('orgId', { plan: 'starter', interval: 'monthly' });
  });
};
