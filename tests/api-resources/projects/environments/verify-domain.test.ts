...
import { BaseVerifyDomain } from '@vaif/api/resources/projects/environments/verify-domain';
...
const runTests = (
  client: PartialVaif<{ projects: { environments: { verifyDomain: BaseVerifyDomain } } }>,
) => {
  test('verifyDomain: only required params', async () => {
    const responsePromise = client.projects.environments.verifyDomain.verifyDomain('envId', {
      projectId: 'projectId',
    });
  ...
  test('verifyDomain: required and optional params', async () => {
    const response = await client.projects.environments.verifyDomain.verifyDomain('envId', {
      projectId: 'projectId',
    });
  });
};
