...
import { BaseMetrics } from '@vaif/api/resources/projects/infrastructure/metrics';
...
const runTests = (client: PartialVaif<{ projects: { infrastructure: { metrics: BaseMetrics } } }>) => {
  test('getMetrics: only required params', async () => {
    const responsePromise = client.projects.infrastructure.metrics.getMetrics('instanceId', { projectId: 'projectId' });
  ...
  test('getMetrics: required and optional params', async () => {
    const response = await client.projects.infrastructure.metrics.getMetrics('instanceId', { projectId: 'projectId' });
  });
};
