...
import { BaseStart } from '@vaif/api/resources/projects/infrastructure/start';
...
const runTests = (client: PartialVaif<{ projects: { infrastructure: { start: BaseStart } } }>) => {
  test('start: only required params', async () => {
    const responsePromise = client.projects.infrastructure.start.start('instanceId', {
      projectId: 'projectId',
    });
  ...
  test('start: required and optional params', async () => {
    const response = await client.projects.infrastructure.start.start('instanceId', {
      projectId: 'projectId',
    });
  });
};
