...
import { BaseRetry } from '@vaif/api/resources/integrations/deliveries/retry';
...
const runTests = (client: PartialVaif<{ integrations: { deliveries: { retry: BaseRetry } } }>) => {
  test('retry', async () => {
    const responsePromise = client.integrations.deliveries.retry.retry('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');
