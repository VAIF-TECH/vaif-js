...
export class BaseVaif {
  ...
  constructor({
    ...
    const options: ClientOptions = {
      ...
      bearerToken,
      ...opts,
      baseURL: baseURL || `https://api.vaif.studio`,
    };
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('VAIF_LOG'), "process.env['VAIF_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('VAIF_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  protected validateHeaders({ values, nulls }: NullableHeaders) {
    if (this.apiKey && values.get('x-vaif-key')) {
      return;
    }
    if (nulls.has('x-vaif-key')) {
      return;
    }
  ...
  protected async apiKeyAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    ...
    return buildHeaders([{ 'x-vaif-key': this.apiKey }]);
  }
...
export declare namespace Vaif {
  ...
  export { Plans as Plans, type PlanRetrieveResponse as PlanRetrieveResponse };
  ...
  export { SchemaEngine as SchemaEngine };
