...
export class BasePlans extends APIResource {
  ...
  retrieve(plan: string, options?: RequestOptions): APIPromise<void> {
    return this._client.get(path`/pricing/plans/${plan}`, { ...options, headers: buildHeaders([{Accept: '*/*'}, options?.headers]) });
  }

  list(options?: RequestOptions): APIPromise<void> {
    return this._client.get('/pricing/plans', { ...options, headers: buildHeaders([{Accept: '*/*'}, options?.headers]) });
  }
}
