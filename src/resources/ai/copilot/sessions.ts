...
export class BaseSessions extends APIResource {
  static override readonly _key: readonly ['ai', 'copilot', 'sessions'] = Object.freeze([
    'ai',
    'copilot',
    'sessions',
  ] as const);
  ...
  retrieve(projectID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.get(path`/ai/copilot/sessions/${projectID}`, {
      ...options,
  ...
  delete(sessionID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.delete(path`/ai/copilot/session/${sessionID}`, {
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
  }
  ...
  retrieve2(sessionID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.get(path`/ai/copilot/session/${sessionID}`, {
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
  }
